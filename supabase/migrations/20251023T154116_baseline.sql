...
begin
  ...
  -- Load item and batch, ensure ownership (RLS still enforced)
  select * into v_item
  ...
  -- Insert cost row
  v_total := round(p_qty * v_item.unit_cost, 2);
...
CREATE OR REPLACE VIEW "public"."v_batch_costs" AS
 SELECT "batch_id",
...
CREATE POLICY "Users can read their own diary entries" ON "public"."diary_entries" FOR SELECT USING (("auth"."uid"() = "user_id"));



CREATE POLICY "batch is owner" ON "public"."plant_batches" USING (("user_id" = "auth"."uid"())) WITH CHECK (("user_id" = "auth"."uid"()));



CREATE POLICY "cost del own" ON "public"."cost_items" FOR DELETE USING (("auth"."uid"() = "user_id"));



CREATE POLICY "cost ins own" ON "public"."cost_items" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "cost is owner" ON "public"."cost_items" USING (("user_id" = "auth"."uid"())) WITH CHECK (("user_id" = "auth"."uid"()));



CREATE POLICY "cost sel own" ON "public"."cost_items" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "diary is owner" ON "public"."diary_entries" USING (("user_id" = "auth"."uid"())) WITH CHECK (("user_id" = "auth"."uid"()));
...
CREATE POLICY "env is owner" ON "public"."environment_logs" USING (("user_id" = "auth"."uid"())) WITH CHECK (("user_id" = "auth"."uid"()));
...
CREATE POLICY "inv del own" ON "public"."inventory_items" FOR DELETE USING (("auth"."uid"() = "user_id"));



CREATE POLICY "inv ins own" ON "public"."inventory_items" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "inv sel own" ON "public"."inventory_items" FOR SELECT USING (("auth"."uid"() = "user_id"));



CREATE POLICY "inv upd own" ON "public"."inventory_items" FOR UPDATE USING (("auth"."uid"() = "user_id")) WITH CHECK (("auth"."uid"() = "user_id"));
...
CREATE POLICY "labor is owner" ON "public"."labor_logs" USING (("user_id" = "auth"."uid"())) WITH CHECK (("user_id" = "auth"."uid"()));
...
CREATE POLICY "profiles are self" ON "public"."profiles" FOR SELECT USING (("id" = "auth"."uid"()));
